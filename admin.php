...
$sql = "SELECT * FROM user" . " WHERE role='1'"; // Assuming role '1' is for users
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                $statusText = $row['status'] == 1 ? "Voted" : "Not Voted";
                $statusClass = $row['status'] == 1 ? "status-voted" : "status-not-voted";
